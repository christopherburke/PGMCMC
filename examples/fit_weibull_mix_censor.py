...
if __name__ == "__main__":
    ...
    etaPar2 = 200.0
    P1 = 1.0/3.0
    censorPrc = 0.5 # 0.1 means treat highest 10% of observations as
                  # not failed yet
...
#    ioblk.fixed[2:5] = 1
    ...
    ioblk.physval_mins = np.array([0.1, 0.01, 0.1, 0.01, 0.01], dtype=float)
    ioblk.physval_maxs = np.array([10.0, 1000.0, 10.0, 1.1e3, 1.0], dtype=float)
    ioblk.calcval_mins = np.array([0.1, -2.0, 0.1, -2.0, 0.01], dtype=float)
    ioblk.calcval_maxs = np.array([10.0, 3.0, 10.0, np.log10(1.1e3), 1.0], dtype=float)
